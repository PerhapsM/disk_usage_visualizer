        ...
        try:
            ...
            if os.path.isdir(item_path):
                ...
                breakdown[item] = {
                    'size': size,
                    'path': item_path,
                    'subdirs': get_folder_breakdown(item_path, max_depth, current_depth + 1)
                }
        except (PermissionError, OSError):
            pass
...
def create_html_report(breakdown: dict, root_path: str, output_file: str = "disk_usage_report.html") -> str:
...
def create_json_report(breakdown: dict, root_path: str, output_file: str = "disk_usage_report.json") -> str:
        ...
        "folders": [
            {
                ...
                "percentage": round((data['size'] / total_size * 100) if total_size > 0 else 0, 2),
                "path": data['path']
            }
...
def main_cli(path: str = None, max_depth: int = 2) -> None:
    ...
    """
    if not path:
        path = input("Enter folder path (or press Enter for current directory): ").strip()
        if not path:
            path = os.getcwd()
            ...
            def build_sunburst_data(bd, root_path=path):
                ...
                def add_folder(folder_name, folder_data, parent_id):
                    ...
                    if folder_data['subdirs']:
                        for subfolder_name, subfolder_data in folder_data['subdirs'].items():
...
def main_gui() -> None:
    ...
    tk.Entry(path_frame, textvariable=selected_path, width=50).pack(side=tk.LEFT, fill=tk.X, expand=True)
    ...
    tk.Scale(root, from_=1, to=5, orient=tk.HORIZONTAL, variable=max_depth_var).pack(fill=tk.X, padx=10)
    
    tk.Button(root, text="Analyze", command=analyze, bg="#667eea", fg="white", font=("Arial", 12, "bold")).pack(pady=20)
