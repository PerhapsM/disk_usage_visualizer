...
if st.session_state.analyze:
    ...
    if not folder_breakdown:
        ...
        sorted_folders = sorted(
            folder_breakdown.items(), key=lambda x: x[1]["size"], reverse=True
        )
        ...
        if not (len(labels) == len(parents) == len(values) == len(ids)):
            st.error(
                "Sunburst data arrays have mismatched lengths; chart will not render."
            )
            valid = False

        if total_values in (0, None):
            st.warning(
                "Sunburst values sum to zero or could not be computed; chart may be empty."
            )
        ...
        if valid and labels and len(labels) > 1 and total_values and total_values > 0:
            ...
            fig_sunburst.update_layout(height=600, font=dict(size=12))
            st.plotly_chart(fig_sunburst, use_container_width=True)
        else:
            st.info(
                "Not enough valid data to display sunburst chart. Check debug output above."
            )
        ...
        for folder_name, folder_data in sorted_folders:
            percentage = (
                (folder_data["size"] / total_size * 100) if total_size > 0 else 0
            )
            table_data.append(
                {
